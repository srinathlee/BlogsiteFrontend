...
const Writeblog = () => {
    ...
    return (
      <div className=" absolute top-0 px-5 sm:px-40 py-5 z-10">
        <div className="p-6 shadow-md bg-white rounded-md flex flex-col gap-3">
          ...
          <img className="h-[70vh]" src={imageUrl || imageUrlai} />
          <p className="font-nato">{data.content}</p>
        </div>
      </div>
    );
  };
  ...
  const PostData = async (event) => {
    ...
    try {
      ...
      data.image = imageUrl || imageUrlai;
      console.log(data);
      const response = await axios.post(`${BASE_URL}/createblog`, data, {
        headers: {
  ...
  const generateImage = async () => {
    ...
    try {
      ...
      console.log(url)
      setImageUrlai(url); // The API directly serves an image
    } catch (err) {
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col items-center">
            <div className="mt-4 w-fit py-4 ">
              ...
              <label
                htmlFor="blogImage"
                className="flex flex-col justify-center items-center border-2  p-1 rounded-md"
              >
                {/* <CiImageOn
              style={{ fontSize: "60px" }}
              ...
              <div className="flex flex-col items-center gap-4 p-4">
                ...
                {imageUrl && (
                  <img
                    src={imageUrlai}
                    alt="Generated"
                    className="mt-4 rounded-xl shadow-md max-w-sm"
                  />
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            <div className="md:col-span-1  col-span-4 relative">
              ...
              <div className="border-r-2 border-b-black border-r-black absolute top-14 md:top-16  right-10 rotate-45 border-b-2 w-3 h-3 ">
                {" "}
              </div>
              {select && (
